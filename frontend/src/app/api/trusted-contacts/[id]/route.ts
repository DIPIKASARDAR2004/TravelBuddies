...
async function getContactId(context: { params: Promise<{ id: string }> }) {
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const id = await getContactId(context);
    ...
    return unauthorizedResponse(error) ?? NextResponse.json({ error: "Unable to update trusted contact." }, { status: 500 });
  }
}
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return unauthorizedResponse(error) ?? NextResponse.json({ error: "Unable to remove trusted contact." }, { status: 500 });
  }
}
